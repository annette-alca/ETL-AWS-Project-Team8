...
def lambda_transform(events, context):
    ...
    for new_key in events["new_keys"]:
        ...
        transformed_name, transformed_df = mvp_transform_df(s3_client, table_name, new_df, processed_bucket)
        list_of_keys.append(save_to_s3(processed_bucket, transformed_df, transformed_name, events["timestamp"]))
...
def save_to_s3(bucket_name, new_df, transform_name, extract_time):
